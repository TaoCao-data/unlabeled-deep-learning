...
import dataset.cifar10_k1 as dataset
...
for epoch in range(epochs):
    ...
    for i in range(n_iter):
        ...
        try:
            U_x1, _ = batched_train_unlabeled_iter.next()
        except:
            batched_train_unlabeled_iter = iter(batched_train_unlabeled)
            U_x1, _ = batched_train_unlabeled_iter.next()
        ...
        with torch.no_grad():
            U_pred1 = model(U_x1)
            #U_pred2 = model(U_x2)
            p = (torch.softmax(U_pred1, dim=1)) / 1
            U_q = p**(1/T) / (p**(1/T)).sum(dim=1, keepdim=True)
            U_q = U_q.detach()
            
        W_x = torch.cat([X_x, U_x1], dim=0)
        W_y = torch.cat([X_y, U_q], dim=0)
