...
import dataset.cifar10_k4 as dataset
...
for epoch in range(epochs):
    ...
    for i in range(n_iter):
        ...
        try:
            (U_x1, U_x2, U_x3, U_x4), _ = batched_train_unlabeled_iter.next()
        except:
            batched_train_unlabeled_iter = iter(batched_train_unlabeled)
            (U_x1, U_x2, U_x3, U_x4), _ = batched_train_unlabeled_iter.next()
        ...
        with torch.no_grad():
            ...
            U_pred3 = model(U_x3)
            U_pred4 = model(U_x4)
            p = (torch.softmax(U_pred1, dim=1) + torch.softmax(U_pred2, dim=1) + torch.softmax(U_pred3, dim=1) + torch.softmax(U_pred4, dim=1)) / 4
            U_q = p**(1/T) / (p**(1/T)).sum(dim=1, keepdim=True)
            U_q = U_q.detach()
            
        W_x = torch.cat([X_x, U_x1, U_x2, U_x3, U_x4], dim=0)
        W_y = torch.cat([X_y, U_q, U_q, U_q, U_q], dim=0)
