...
from utils import wrn, transform
...
if args.alg == "VAT": # virtual adversarial training
    ...
    from models.pseudo_label import PL
    ssl_obj = PL(alg_cfg["threashold"])
elif args.alg == "MT": # mean teacher
    from models.mean_teacher import MT
    ...
    from models.pimodel import PiModel
    ssl_obj = PiModel()
    ...
    if (iteration % args.validation) == 0 or iteration == shared_cfg["iteration"]:
        with torch.no_grad():
            ...
            if maximum_val_acc < acc:
                ...
                torch.save(model.state_dict(), os.path.join(args.output, "best_model_pl2000.pth"))
                # torch.save(model.state_dict(), 'pl4000.ckpt')
        model.train()
        s = time.time()
    # lr decay
