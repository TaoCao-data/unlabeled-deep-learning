...
class SemiLoss(object):
    def __call__(self, pred_x, targets_x, pred_u, targets_u, epoch, lambda_u, epochs):
        probs_u = torch.softmax(pred_u, dim=1)

        Lx = -torch.mean(torch.sum(F.log_softmax(pred_x, dim=1) * targets_x, dim=1))
        Lu = torch.mean((probs_u - targets_u)**2)

        return Lx, Lu, lambda_u * np.minimum(1.0, np.maximum(epoch/epochs, 0.0))
...
for epoch in range(epochs):
    ...
    for i in range(n_iter):
        ...
        if if_cuda:
            X_x, X_y = X_x.cuda(), X_y.cuda(non_blocking=True)
            U_x1 = U_x1.cuda()
            U_x2 = U_x2.cuda()
        ...
        with torch.no_grad():
            ...
            pt = p**(1/T)
            U_q = pt / pt.sum(dim=1, keepdim=True)
            U_q = U_q.detach()
        ...
        Lx, Lu, w = criterion(logits_x, Mix_y[:batch_size], logits_u, Mix_y[batch_size:], epoch + i/n_iter, lambda_u, epochs)
